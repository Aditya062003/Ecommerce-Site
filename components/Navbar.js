  ...
  return (
    <div className='sticky top-0 z-10 bg-white flex justify-center md:flex-row md:justify-start flex-col items-center py-1 mb-1 shadow-md'>
      ...
      <div ref={ref} className={`w-[36vh] h-[100vh] fixed px-6 py-10 sidecart top-0 right-0 p-10 bg-blue-50 transform transition-transform ${Object.keys(cart).length!==0?'translate-x-0':'translate-x-full'}`}>
        ...
        <ol>
          {Object.keys(cart)==0 && <div className='my-2 text-center font-semibold '> Your Cart is Empty.</div>}
          {Object.keys(cart).map((k)=>{ return <li key={k}>
            <div className="flex my-4">
            <span className='w-2/3 font-semibold'>{cart[k].name}</span>
            <span className='w-1/3 font-semibold flex items-center justify-center'><AiOutlineMinusCircle className='mx-2 cursor-pointer text-3xl' onClick={()=>{removeFromCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant)}}/> {cart[k].qty} <AiOutlinePlusCircle onClick={()=>{addToCart(k,1,cart[k].price,cart[k].name,cart[k].size,cart[k].variant)}} className='mx-2 text-3xl cursor-pointer'/></span>
            </div>
          </li>})}
        </ol>
        <div className="flex justify-center">
        <Link href={'/checkout'}><button class="flex mx-2  text-white bg-indigo-500 border-0 py-2 px-2 focus:outline-none hover:bg-indigo-600 rounded text-sm"><BsFillBagCheckFill className='m-1'/>Checkout</button></Link>
        <button onClick={clearCart} class="flex mx-2 text-white bg-indigo-500 border-0 py-2 px-2 focus:outline-none hover:bg-indigo-600 rounded text-sm">Clear Cart</button>
        </div>
      </div>
    </div>
